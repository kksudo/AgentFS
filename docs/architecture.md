...
# AgentOS: Architectural Design Document

> "Плохие программисты думают о коде. Хорошие программисты думают о структурах данных и их связях." — Linus Torvalds

> Задача: спроектировать CLI-инструмент `npx create-agentos`, который разворачивает Obsidian vault как операционную систему для AI-агента. Файловая система — единственный интерфейс. Никаких баз данных. Никаких фреймворков.
...
- **Kernel space** — единый source of truth. Компилирует конфиги в нативные форматы через `agentos compile`
...
└─────────────────┘ │   memory.json│
                    │   (synced)   │
                    └──────────────┘
         + AGENT-MAP.md (vault router, generated from manifest)
```

**Правила compile:**

1. **`agentos compile`** — читает `manifest.yaml` + `init.d/` + `memory/`, генерирует нативные конфиги
...
|-----------------|-------|-----------|------------|
| `.agentos/manifest.yaml` | AgentOS | `agentos init`, пользователь | `agentos compile` |
| `.agentos/init.d/*.md` | AgentOS | `agentos init`, пользователь | `agentos compile` |
| `.agentos/memory/` | AgentOS | агенты (через consolidation) | `agentos compile`, агенты |
| `CLAUDE.md` | AgentOS (compiled) | `agentos compile` | Claude Code |
...
| `.claude/skills/` | AgentOS (compiled) | `agentos compile` | Claude Code |
| `.omc/project-memory.json` | AgentOS (synced) | `agentos compile` | OMC / OpenClaw |
| `.omc/sessions/` | OMC | OMC runtime | OMC |
| `.omc/state/` | OMC | OMC runtime | OMC |
| `.cursorrules` | AgentOS (compiled) | `agentos compile` | Cursor |
| `.cursor/` | Cursor | Cursor | Cursor |

4. **Bidirectional sync для memory:** `.agentos/memory/semantic.md` — canonical source. При compile → пишет в `.omc/project-memory.json`. При import → читает из `.omc/project-memory.json` новые факты обратно в canonical. `agentos sync` также отслеживает drift между compiled outputs (CLAUDE.md) и manifest — если CLAUDE.md отредактирован вручную, предлагает обновить manifest.

```bash
agentos compile              # manifest → все нативные форматы
agentos compile claude       # только Claude конфиги
agentos compile openclaw     # только OpenClaw конфиги
agentos compile --dry-run    # показать что изменится, не писать
agentos import memory        # импорт памяти из нативных форматов в canonical
```
...
├── CLAUDE.md                    ← compiled output ← agentos compile claude
├── MEMORY.md                    ← compiled output ← agentos compile openclaw (or symlink)
├── AGENT-MAP.md                 ← compiled output ← agentos compile (vault router)
└── .cursorrules                 ← compiled output ← agentos compile cursor
```
...
Creating vault structure (profile: personal)...
  ...
  ✓ Compiled .agentos/ → CLAUDE.md (artifact, see: agentos compile claude)
  ✓ Compiled .agentos/ → MEMORY.md (artifact, see: agentos compile openclaw)
...
```bash
# Управление vault
agentos status                    # показать текущее состояние
agentos doctor                    # проверить целостность vault
agentos migrate                   # миграция из старой структуры

# Профили агентов
agentos profile generate claude   # перегенерировать CLAUDE.md
agentos profile generate openclaw # перегенерировать SOUL.md
agentos profile list              # показать все agent profiles

# Memory
agentos memory show               # показать semantic memory (facts, prefs, patterns)
agentos memory consolidate        # ручной запуск consolidation
agentos memory search "kubernetes" # поиск по памяти

# Inbox
agentos triage                    # запустить inbox triage
agentos triage --auto             # автоматическая классификация

# Modules
agentos module add career         # добавить модуль
agentos module remove clients     # убрать модуль
agentos module list               # список активных модулей

# Onboard (agent-led interview → identity + memory)
agentos onboard                   # запустить интервью: стек, роли, предпочтения
                                  # ответы → init.d/00-identity.md + memory/semantic.md
                                  # Phases: SCAFFOLD (create-agentos) → PERSONALIZE (onboard) → COMPILE

# Sync (bidirectional manifest ↔ compiled outputs)
agentos sync                      # если CLAUDE.md отредактирован вручную → diff → обновить manifest
agentos sync --check              # показать drift (что разошлось) без изменений
agentos sync --cross-vault ~/work-notes  # синхронизировать shared standards между vault-ами

# Security (AppArmor profiles)
agentos security mode enforce     # enforce | complain | disabled
agentos security audit            # показать violations.log
agentos security test             # dry-run: проверить policy на конфликты
agentos security add crypto       # добавить domain-specific security module
agentos security add web          # модули: crypto, web, infra, cloud, ci-cd
agentos security list             # показать активные security modules

# Secrets (reference-only vault)
agentos secret add <name>         # добавить секрет
agentos secret list               # показать refs (без значений)
agentos secret rotate <name>      # обновить значение
agentos exec --with-secrets "cmd" # выполнить команду с подстановкой секретов
```
...
### AGENT-MAP.md — compiled markdown router

Помимо per-agent конфигов, `agentos compile` генерирует `AGENT-MAP.md` — человекочитаемый индекс всего vault. Идея из [Agent-Scaffolding](https://github.com/DOWingard/Agent-Scaffolding): вместо того чтобы агент сканировал весь vault, дать ему один файл-маршрутизатор.

**Input:** `manifest.yaml` (paths, modules, boot sequence)
**Output:** `AGENT-MAP.md` (корень vault)

```markdown
# Agent Map — Auto-generated by agentos compile
...
- Memory sync двунаправленный: `agentos compile openclaw` пишет canonical → `.omc/`, а `agentos import memory` читает `.omc/` → canonical.
...
│   │   ├── exec-proxy.ts         ← agentos exec --with-secrets runtime
...
│   │   ├── compile.ts            ← agentos compile [agent] [--dry-run]
│   │   ├── import.ts             ← agentos import memory
│   │   ├── status.ts
│   │   ├── doctor.ts             ← vault checks + agnix integration (385 lint rules)
│   │   ├── triage.ts
│   │   ├── migrate.ts
│   │   ├── memory.ts
│   │   ├── onboard.ts            ← agentos onboard (agent-led interview)
│   │   ├── sync.ts               ← agentos sync (bidirectional manifest ↔ compiled)
│   │   ├── secrets.ts            ← agentos secret add|remove|list|rotate|inject
│   │   ├── security.ts           ← agentos security mode|audit|test|add|remove|list
│   │   └── exec.ts               ← agentos exec --with-secrets (proxy)
│   └── utils/
│       ├── frontmatter.ts
...
|-----------|-------|-----------------|
| `agnix` | 385 правил валидации для CLAUDE.md, AGENTS.md, SKILL.md, hooks | `agentos doctor` = наши vault-проверки + `agnix lint` |
...
- `agentos doctor` запускает три набора проверок:
...
- В CI: `agentos doctor` на каждый commit (включает все три набора)
...
5. **Идемпотентность.** `npx create-agentos` на существующий vault — безопасно. Не перезаписывает пользовательские файлы, только обновляет `.agentos/`.
...
### agentos migrate (CLI — будущая автоматизация)

```bash
$ agentos migrate --source ~/notes --profile personal
...
`.agentignore` — аналог `.dockerignore`. Soft deny: convention, не enforcement. Но `agentos compile` вставляет его в CLAUDE.md как жёсткие инструкции.
...
Если нужен секрет — используй `agentos secret get <name>`, НЕ читай файл напрямую.
...
input_validation:
  ...
  # agentos doctor включает scan всего vault на эти паттерны
...
```json
// .claude/settings.json — GENERATED by agentos compile security
{
  "permissions": {
    "allow": [],
    "deny": [
...
```bash
agentos security mode enforce     # включить enforce
agentos security mode complain    # включить complain (логирование)
agentos security audit            # показать violations.log
agentos security test             # dry-run: проверить policy на конфликты
```
...
| **AgentOS** | `${{secret:name}}` — `agentos exec` подставляет, агент не видит |
...
└── README.md                ← инструкция для агента: "используй agentos secret get"
```
...
# Для использования: agentos secret get <name>
...
# Расшифровывается только через agentos secret get
...
Правильно (AgentOS way):
  ...
  2. Агент вызывает: agentos exec --with-secrets "curl -H 'Authorization: Bearer ${{secret:github-pat}}' https://api.github.com/..."
  3. CLI `agentos exec`:
...
```bash
# Управление
agentos secret add aws-access-key --type api-key --scope "Projects/{project-a}/"
agentos secret remove smtp-password
agentos secret list                    # показать refs (без значений)
agentos secret rotate github-pat       # перегенерировать/обновить

# Использование (агент вызывает эти команды)
agentos exec --with-secrets "command with ${{secret:name}}"
agentos secret inject .env.template .env   # подставить секреты в шаблон → файл

# Аудит
agentos secret audit                   # кто когда обращался к каким секретам
```
...
Отдельный compile-time guard. При `agentos compile` сканируются ВСЕ compiled outputs (CLAUDE.md, SOUL.md, .cursorrules) на наличие паттернов секретов:
...
```bash
agentos security add crypto       # активировать модуль для этого vault
agentos security add web infra    # несколько сразу
agentos security remove crypto    # деактивировать
agentos security list             # показать base + активные модули
```

При `agentos compile security` — base policy мержится с активными модулями → единый enforcement profile. Порядок: base → modules (modules расширяют, не перезаписывают).
...
│  Raw values → только через agentos exec --with-secrets        │
...
3. **`agentos exec --with-secrets` требует local runtime.** Если агент работает в cloud (Cowork VM) — age key должен быть доступен в environment. Это отдельная задача для deployment.
...
3. **Git integration.** Должен ли `create-agentos` инициализировать git? `.gitignore` для `.agentos/proc/` (ephemeral state)? Или это решение пользователя?
...
5. **Compile triggers.** Когда запускать `agentos compile` автоматически? При каждом `git commit`? При изменении `manifest.yaml`? Через file watcher? Или только вручную?
...
Phase 2:   MVP — npx create-agentos: personal profile + compile.d/claude + AGENT-MAP.md
Phase 2.5: Onboard — agentos onboard (agent-led interview → identity + memory)
...
Phase 6:   Secrets vault — SOPS/age + agentos exec --with-secrets + regex guard
Phase 7:   Sync — agentos sync (bidirectional manifest ↔ CLAUDE.md) + cross-vault
